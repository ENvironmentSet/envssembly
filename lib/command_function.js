...
var commands = ( (Memory,Register,char_table) => {
    ...
    var emu_call_table = {
        ...
        "2":function input_cmd (saveAble,size) {
            let answer = prompt('').intCarr();
            if(answer.length > size) throw new RuntimeException("input is bigger than buffer");
            if(type.is_mem(saveAble)) {
                let address = util_getMemoryAdress(saveAble);
                let item;
                while(item = answer.pop()) {
                    Memory.set_byte(item,address++);
                }
                Memory.set_byte(0,address++);
            } else if(typeof saveAble === "number") {
                let item;
                while(item = answer.pop()) {
                    Memory.set_byte(item,saveAble++);
                }
                Memory.set_byte(0,saveAble++);
            } else throw new RuntimeException("Register dx doesn't point memory");
        },
    ...
    function lea (op1,op2) {
        if(type.is_Register(op1)) {
            if(char_table.has(op2)) {
                Register[op1] = char_table.get(op2);
            } else throw new RuntimeException("operator 2 is not a Memory");
        } else throw new RuntimeException("operator 1 is not a Register")
    }
