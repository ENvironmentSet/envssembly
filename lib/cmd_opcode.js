...
var opcode_t = ( (char_table,RunnerException) => {
    ...
    * */
    let opcode_table = ["blank","ax","bx","cx","dx","si","di","bp","sp","ip","cs","ds","bs","ss",
        ...
        "cmd_end","8bitI","16bitI","STRING","[memory_exp]"];
    ...
    function encode (token) {
        ...
        if(type.is_StringL(token.left)) {
            let operand = token.right.substr(1,token.left.length-2).intCarr(); // remov ""
            ...
            while(operand.check()) {
                ...
                if(type.is_digitL(item)) {
                    item >>= 0;
                    if(item <= 255) { // 8bit
                        result.push(252,item & 0xFF);
                    } else if (item <= 65535) { // 16bit
                        result.push(253,(item >> 8) & 0xFF,item & 0xFF);
                    } else throw new RunnerException("Number is Bigger than 16-bit limit token :"+JSON.stringify(token));
                } else if (char_table.has(item)) {
                    let a = char_table.get(item);
                    result.push(253,(a >> 8) & 0xFF,a & 0xFF,0);
                } else {
        ...
        if(type.is_StringL(token.right)) {
            let operand = token.right.substr(1,token.right.length-2).intCarr(); // remov ""
            result.push(254,...operand,0);
    ...
    function fetch (arr) {
        ...
        while((item = arr.shift()) != 0) {
            if(item === undefined) throw new RunnerException("command fetch error, infinity-length cmd");
            items.push(item);
        }
        return items;
    }
