    ...
    * */
    ...
    function encode (token) {
        ...
        if(type.is_StringL(token.left)) {
            ...
            let adr = char_table.get(token.left);
            if(adr <= 255) { // 8bit
                result.push(250,adr & 0xFF,0);
            } else if (adr <= 65535) { // 16bit
                result.push(250,(adr >> 8) & 0xFF,adr & 0xFF,0);
            } else throw new OpcodeException("Number is Bigger than 16-bit limit token :"+JSON.stringify(token));
        } else result.push(0);
        ...
        if(type.is_StringL(token.right)) {
            ...
            let adr = char_table.get(token.right);
            if(adr <= 255) { // 8bit
                result.push(250,adr & 0xFF,0);
            } else if (adr <= 65535) { // 16bit
                result.push(250,(adr >> 8) & 0xFF,adr & 0xFF,0);
        ...
        while(op_arr.length) {
            ...
            if(item === 252) { //8bit Num
                ...
                switch (flag) {
                    ...
                    case "char_variable":
                        varif = true;
                        if(op_arr[0] != 0) result = (item << 8) + op_arr.shift();
                        result = item;
                        break;
                    default : // mne
                            result = opcode_table[item];
